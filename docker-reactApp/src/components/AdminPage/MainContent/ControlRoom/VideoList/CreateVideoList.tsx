...
export default function CreateVideoList() {
  ...
  const deleteVideo = async (
    ...
    if (!window.confirm(`Delete video ${Object.keys(selectedVideo)[0]} ?`)) return;
  ...
  const liveIconDisplay = (key: number, item: SelectedVideoProps, liveVideoUrl:string) => {
  ...
  useEffect(() => {
    chooseVideo(0, videoFullList);
    setSelectedVideo({ main: "https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8" });
  }, []);

  const listGroup = videoFullList.map((item: SelectedVideoProps, key: number) => {
