...
const Login = (props: LoginSubmitProps) => {
  ...
  const [loginTitleClassName, setloginTitleClassName] = useState(`${classes.loginTitle}`);
  ...
  const loginValidation = async (loginUserData: LoginFormData) => {
    ...
    if (response.data.message === true) {
      ...
      setloginTitleClassName(`${classes.loginTitle} ${classes.loginTitleMargin}`);
  ...
  return (
    <div className={classes.loginBackground}>
      <div className={classes.loginContainer}>
        <Form onSubmit={(event) => handleSubmit(event, loginUserData)} className="w-100">
